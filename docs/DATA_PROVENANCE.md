...
The workbook contains only expert identifiers, criterion-pair labels, alternative labels and numerical scores. Its two sheets are visible. No cell comments, external workbook links, participant names, email addresses, demographic records or free-text participant responses are included. Workbook creator metadata identifies the writing library, not a participant. The identifiers permit pairing each expert's AHP matrix with that expert's R1 scores; no identifying key is included.

This describes the files supplied for release. It does not independently authenticate how responses were collected or establish the adequacy of consent documentation. Original questionnaires and consent records are not included. Public release does not resolve the absence of individual R2–R6 responses.
